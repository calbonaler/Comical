...
public class Comic : IDisposable, INotifyPropertyChanged
{
	...
	async Task<FileHeader> ReadFileAsync(string fileName, BookmarkCollection bookmarks, IProgress<int> progress)
	{
		...
		var fileHeader = await FileHeader.LoadAsync(stream).ConfigureAwait(false) ?? throw new ArgumentException(Resources.InvalidFileFormat);
	...
	public async Task SaveAsync(string fileName, IProgress<int> progress)
	{
		...
		{
			if (IsInconsistent())
				throw new InconsistentDataException(Resources.InconsistentData);
			var header = new FileHeader(Title, Author, Published, BindingSide, Thumbnail);
			await WriteFileAsync(fileName, header, [.. Images], Bookmarks, progress).ConfigureAwait(false);
	...
	{
		using (EnterSingleOperation())
			await WriteFileAsync(fileName, new FileHeader(string.Empty, Author, null, BindingSide, null), [.. images], [], progress).ConfigureAwait(false);
	}
	...
	{
		if (IsBusy)
			throw new InvalidOperationException(Resources.MultiAsyncOperationIsNotSupported);
		IsBusy = true;
